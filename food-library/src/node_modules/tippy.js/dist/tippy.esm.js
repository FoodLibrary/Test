      ...
      onTrigger: function onTrigger(_, event) {
        if (isMouseEvent(event)) {
          ...
          var cursorRect = rects.find(function (rect) {
            return rect.left - 1 <= event.clientX && rect.right + 1 >= event.clientX && rect.top - 1 <= event.clientY && rect.bottom + 1 >= event.clientY;
          });
...
function getInlineBoundingClientRect(currentBasePlacement, boundingRect, clientRects, cursorRectIndex) {
  ...
  if (clientRects.length === 2 && cursorRectIndex !== null && clientRects[0].left > clientRects[1].right) {
